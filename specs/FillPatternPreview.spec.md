...
## 11. Testing
There is currently no automated test project in this repository. Verification during development has been manual: loading sample `.pat` text into `samples/PatternPreviewSampleApp` and visually comparing the rendered output against an independent reference `.pat` preview tool, for:
- Single-family solid lines at 0°, 45°, and 90° (regression case for the shift/offset rotation bug, §4).
- A ~80-family real-world "English bond brick detail" pattern mixing solid and dashed families at 0°/90°/180°/-90° (regression case for the dash-phase-anchor and intersection-ordering bugs, §8).
